...
caminhar(X,Y,H,C) :- alimento(X,Y), customaximo(CAM), C < CAM, retract(customaximo(CAM)), assert(customaximo(C)), assert(bicho(X,Y)), assert(mente((X,Y),alimento)), /* tty_clear, */ writeln(''), writeln(''), write('Custo: '), write(C), write(', Caminho: '), adiciona((X,Y),H,H1), writeln(H1), mapa(1,1,H), fail;
