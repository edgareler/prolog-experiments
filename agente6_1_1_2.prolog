...
listaPertence([X|R],L) :- pertence(X,L), listaPertence(R,L).
listaPertence([],_).
...
caminhar(X,Y,H) :- % sleep(1),
    ...
    */
        ...
        melhorCaminhoNo(MCN), length(MCN,MCNL), writeln(MCN), writeln(H), MCNL > 0, listaPertence(MCN,H), writeln('Melhor caminho: '), writeln(MCN), writeln('Está contido no Histórico: '), writeln(H),
        customaximo(CAM), write('Custo máximo: '), writeln(CAM), sleep(5), 
        (
    ...
    (
        ...
        (
            noMelhorCaminho(X,Y), writeln('$$$ Nó Melhor Caminho $$$'), caminhosPossiveis([[CN|_]|_]), custoNoMelhorCaminho(CNM), CN < CNM, retract(custoNoMelhorCaminho(CNM)), assert(custoNoMelhorCaminho(CN)), assert(bicho(X,Y)), assert(mente((X,Y),melhorCaminho)),
            retractall(melhorCaminhoNo(_)), assert(melhorCaminhoNo(H)), writeln(H), sleep(5); true
        ),
